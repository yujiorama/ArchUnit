...
class ClassFileProcessor {
    ...
    private static class RecordAccessHandler implements AccessHandler {
        ...
        public void handleMethodReferenceInstruction(String owner, String name, String desc) {
            ...
            if (CONSTRUCTOR_NAME.equals(name)) {
                importRecord.registerConstructorReferenceCall(filled(new RawAccessRecord.Builder(), targetInfo).build());
            } else {
                importRecord.registerMethodReferenceCall(filled(new RawAccessRecord.Builder(), targetInfo).build());
            }
        }
